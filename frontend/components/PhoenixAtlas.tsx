...
export default function Atlas(){
 ...
 const [fs,setFs]=useState<F[]>([]),[proj,setProj]=useState<any[]>([]),[metric,setMetric]=useState<Metric>("teu"),[mode,setMode]=useState<Mode>("cells"),[sel,setSel]=useState<F|null>(null),[full,setFull]=useState(false),[city,setCity]=useState<any>(null),[error,setError]=useState<string|null>(null);
 ...
 useEffect(()=>{if(!host.current)return;const map=new maplibregl.Map({container:host.current,center:[-112.074,33.448],zoom:10.4,style:{version:8,sources:{osm:{type:"raster",tiles:["https://tile.openstreetmap.org/{z}/{x}/{y}.png"],tileSize:256,attribution:"© OpenStreetMap contributors"}},layers:[{id:"osm",type:"raster",source:"osm",paint:{"raster-opacity":.62,"raster-saturation":-.22}}]} as any,attributionControl:false});mapRef.current=map;map.addControl(new maplibregl.NavigationControl(),"top-right");map.addControl(new maplibregl.AttributionControl({compact:true}),"bottom-right");
  map.once("load",async()=>{try{const fc:any=await E.cells();const a:F[]=JSON.parse(JSON.stringify(fc.features??[]));if(!a.length)throw new Error("No verified spatial cells returned");setFs(a);const pts=a.flatMap(ring);map.fitBounds([[Math.min(...pts.map(p=>p[0])),Math.min(...pts.map(p=>p[1]))],[Math.max(...pts.map(p=>p[0])),Math.max(...pts.map(p=>p[1]))]],{padding:90,duration:0,maxZoom:15.2});setTimeout(()=>project(a),80);try{const r=await fetch("/data/phoenix_boundary.geojson");if(r.ok)setCity(await r.json())}catch{}}catch(e:any){setError(e.message)}});
 ...
 return <div className="atlas-operational" ref={wrap}>
  <div className="atlas-topline"><div><div className="eyebrow">PHOENIX · PLANNING MODE · SPATIAL INTELLIGENCE</div><h1>Thermal Atlas</h1><p>Downtown operational AOI by default, with truthful Phoenix-wide context on demand.</p></div><div className="atlas-health"><span className={fs.length===4?"live":"warn"}><i/>{fs.length===4?"LIVE PROVIDER DATA":"DATA CHECK"}</span><span>{fs.length||"—"} VERIFIED CELLS</span><span>LIVE AOI</span></div></div>
  ...
  <div className="atlas-map-wrap"><div ref={host} className="atlas-map-full"/>{error&&<div className="atlas-error">Backend unavailable: {error}. Reload to retry.</div>}
   <div className="phoenix-overlay-note"><b>PHOENIX</b><span>Citywide OSM operating context</span><i>Live provider validation footprint shown separately</i></div>
   <svg className="helios-svg-overlay">
    <defs><filter id="hb"><feGaussianBlur stdDeviation="34"/></filter></defs>
    {mode==="coverage"&&proj.map(({pts,f},i)=><g key={i} className="geo-cell" onClick={()=>setSel(f)}><polygon points={pts.map((p:number[])=>p.join(",")).join(" ")} fill="#d9ff53" fillOpacity=".31" stroke="#d9ff53" strokeWidth="4" strokeDasharray="12 8"/></g>)}
    {mode==="cells"&&proj.map(({pts,f},i)=>{const v=Number(f.properties[metric]);return <g key={i} className="geo-cell" onClick={()=>setSel(f)}><polygon points={pts.map((p:number[])=>p.join(",")).join(" ")} fill={c(v,def)} fillOpacity=".80" stroke="#07100c" strokeWidth="7"/><polygon points={pts.map((p:number[])=>p.join(",")).join(" ")} fill="none" stroke="#efff9b" strokeWidth="2.5"/></g>})}
    ...
    {mode==="threeD"&&proj.map(({pts,q,f},i)=>{const v=Number(f.properties[metric]),h=35+norm(v)*120,top=pts.map((p:number[])=>[p[0],p[1]-h]);return <g key={i} className="geo-cell" onClick={()=>setSel(f)}>{pts.slice(0,-1).map((p:number[],j:number)=>{const q2=pts[j+1],a=top[j],b=top[j+1];return <polygon key={j} points={`${p[0]},${p[1]} ${q2[0]},${q2[1]} ${b[0]},${b[1]} ${a[0]},${a[1]}`} fill={c(v,def)} fillOpacity=".58" stroke="#07100c" strokeWidth="2"/>})}<polygon points={top.map((p:number[])=>p.join(",")).join(" ")} fill={c(v,def)} fillOpacity=".94" stroke="#efff9b" strokeWidth="3"/><text x={q[0]} y={q[1]-h-8} textAnchor="middle" className="svg-cell-label">{f.properties.cell_id}</text></g>})}
   </svg>
   <div className="coverage-card"><div className="eyebrow">COVERAGE CONTRACT</div><h3>{mode==="coverage"?"Live provider footprint":"Phoenix + verified live AOI"}</h3><p>Only the highlighted downtown cells carry current provider-backed operational metrics. Unobserved Phoenix is intentionally not assigned fabricated values.</p><button onClick={liveView}>Zoom to live AOI</button></div>
