...
export default function AgentRoom(){
 ...
 const actions=Array.isArray(rec?.recommended_actions)?rec.recommended_actions:[];
 ...
 {runs.loading||latest.loading?<div className="empty compact-empty">Loading governed agent evidence…</div>:runs.error||latest.error?<div className="errorbox">Agent evidence unavailable: {runs.error??latest.error} <button onClick={()=>{runs.retry();latest.retry()}}>Retry</button></div>:<>
  ...
  <div className="agent-grid"><section><div className="section-kicker">LATEST RECOMMENDATION</div><h3>{s(rec,"headline")}</h3><div className="agent-actions">{actions.length?actions.map((x:any,i:number)=>{const label=s(x,"action","cell_id");return <div key={i}><b>{label!=="—"?label:`Action ${i+1}`}</b><span>{s(x,"reason","rationale","status")}</span></div>}):<div className="empty compact-empty">No structured recommended-action list in the latest governed record.</div>}</div></section><section><div className="section-kicker">SKEPTIC FINDINGS</div><h3>What could invalidate the decision?</h3><pre className="clean-pre">{JSON.stringify(rec?.skeptic_findings??summary?.skeptic_findings??{},null,2)}</pre></section></div>
